...
// Function to generate failure frames randomly
void generateFailureFramesRandom(std::vector<int> &failure_frames, int min, int max, double fractionToFail)
{
    ...
    for (int i = 0; i < numFail; i++)
    {
        while (!uniqueFound)
        {
            ...
            for (int j = 0; j < failure_frames.size(); j++)
            {
                if (val == failure_frames[j])
                {
                    uniqueFound = false;
                    break;
                }
...
int main(int argc, char **argv)
{
   ...
   ros::Rate poll_rate(100);
	...
	std::vector<int> failure_frames;
    generateFailureFramesRandom(failure_frames, 0, nImages, 0.0);


	for (size_t imgId = 0; imgId < nImages; imgId++)
    {
        // Skip feature extraction if imgId is in the failure frames list
        if (std::find(failure_frames.begin(), failure_frames.end(), imgId) != failure_frames.end())
        {
            std::cout << "Skipping feature extraction for image " << imgId << std::endl;
            continue;
        }
